...
// start transaction
// select
// (select exist (select 1 from accounts where id = 1)) as from,
// (select exist (select 2 from accounts where id = 1)) as to,
// (select balance from accounts where id = 1) as bal
// info @from, @to,@bal
